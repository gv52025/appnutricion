...
${suffix}`;
  ...
  function record(row2) {
    return { ...row2.body, id: row2.id, kind: row2.kind, rev: row2.revision, patient: row2.patient_id, created: row2.created_at, updated: row2.updated_at };
  }
  async function row(id) {
    return unwrap(await client.from("records").select("*").eq("workspace_id", workspace).eq("id", id).single());
  }
  async function demo(patient) {
    const p = await row(patient);
    if (p.kind !== "patient" || p.body?.demo !== true) throw new Error("El modo abierto solo admite expedientes marcados como ficticios.");
    return p;
  }
  async function insert(kind, body, patient = null) {
    const value = unwrap(await client.from("records").insert({ workspace_id: workspace, kind, patient_id: patient, body, created_by: (await client.auth.getUser()).data.user.id, updated_by: (await client.auth.getUser()).data.user.id }).select().single());
    return record(value);
  }
  async function update(id, changes) {
    const old = await row(id), body = { ...old.body, ...clean(changes) };
    const value = unwrap(await client.from("records").update({ body }).eq("workspace_id", workspace).eq("id", id).eq("revision", changes.rev || old.revision).select().single());
    return record(value);
  }
  async function cloudApi(path, data) {
    if (path === "bootstrap") {
    ...
    if (!session) throw new Error("Inicia sesi\xF3n en Supabase.");
    if (path === "state") {
      const member = await context();
      const [rowsResult, profileResult, settingsResult, audioResult, transcriptResult] = await Promise.all([
        client.from("records").select("id,kind,patient_id,revision,body,created_at,updated_at").eq("workspace_id", workspace).order("created_at", { ascending: false }),
        client.from("profiles").select("display_name,professional_credential,contact").eq("user_id", session.user.id).maybeSingle(),
        client.from("app_settings").select("key,value").eq("workspace_id", workspace),
        client.from("consultation_recordings").select("*").eq("workspace_id", workspace).order("created_at", { ascending: false }),
        client.from("consultation_transcripts").select("*").eq("workspace_id", workspace).order("created_at", { ascending: false })
      ]);
      const rows = unwrap(rowsResult) || [], profile = unwrap(profileResult) || {}, settingRows = unwrap(settingsResult) || [];
    ...
    if (path === "visits") {
      ...
      const row2 = unwrap(await client.rpc("create_visit", { p_patient: data.patient, p_date: data.date, p_reason: data.reason, p_type: data.type || "Primera consulta" }));
      return record(one(row2));
    }
    if (path === "recordings") {
      await demo(data.patient);
      const file = data.file;
      if (!(file instanceof File) || !file.size) throw new Error("Selecciona un audio v\xE1lido.");
      if (file.size > 52428800) throw new Error("El audio supera el m\xE1ximo de 50 MB.");
      const allowed = ["audio/mpeg", "audio/mp4", "audio/x-m4a", "audio/wav", "audio/x-wav", "audio/ogg", "audio/webm", "audio/flac"];
      const ext = (file.name.split(".").pop() || "").toLowerCase(), fallback = { mp3: "audio/mpeg", m4a: "audio/x-m4a", mp4: "audio/mp4", wav: "audio/wav", ogg: "audio/ogg", webm: "audio/webm", flac: "audio/flac" }[ext];
      const mime = allowed.includes(file.type) ? file.type : fallback;
      if (!mime) throw new Error("Formato no compatible. Usa MP3, M4A, WAV, OGG, WEBM, FLAC o MP4.");
      const sha2562 = [...new Uint8Array(await crypto.subtle.digest("SHA-256", await file.arrayBuffer()))].map((x) => x.toString(16).padStart(2, "0")).join("");
      const storagePath = `${workspace}/${data.patient}/${data.visit}/${crypto.randomUUID()}.${ext}`;
      unwrap(await client.storage.from("consultation-audio").upload(storagePath, file, { contentType: mime, upsert: false }));
      let saved;
      try {
        saved = unwrap(await client.from("consultation_recordings").insert({ workspace_id: workspace, patient_id: data.patient, visit_id: data.visit, storage_path: storagePath, original_name: file.name, mime_type: mime, size_bytes: file.size, sha256: sha2562, recorded_at: data.recorded_at || null, consent_confirmed: true, consent_method: data.consent_method, consent_statement: data.consent_statement, consent_at: (/* @__PURE__ */ new Date()).toISOString(), retention_until: data.retention_until || null, status: "queued", created_by: session.user.id }).select().single());
      } catch (error) {
        await client.storage.from("consultation-audio").remove([storagePath]);
        throw error;
      }
      const invoked = await client.functions.invoke("transcribe-consultation", { body: { recording_id: saved.id } });
      if (invoked.error) throw new Error("El audio qued\xF3 guardado, pero la transcripci\xF3n no inici\xF3: " + invoked.error.message);
      if (invoked.data?.error) throw new Error(invoked.data.error);
      return { ok: true, id: saved.id };
    }
    if (path === "transcripts/review") {
      return one(unwrap(await client.rpc("review_consultation_transcript", { p_id: data.id, p_status: data.status, p_note: data.note, p_text: data.text })));
    }
    if (path === "visits/save") {
    ...
    if (path === "visits/close") {
      const old = await row(data.id);
      await demo(old.patient_id);
      return update(data.id, { ...data, status: "closed", closed_at: (/* @__PURE__ */ new Date()).toISOString() });
    }
    ...
    if (path === "demo/estela") {
      const result = await client.functions.invoke("seed-estela-demo", { body: data });
      if (result.error) {
        let detail = "";
        try {
          detail = (await result.error.context?.json())?.error || "";
    ...
    if (path === "documents") {
      ...
      const storagePath = `${workspace}/${data.patient}/${crypto.randomUUID()}.${ext}`;
      unwrap(await client.storage.from("clinical-documents").upload(storagePath, new Blob([binary], { type: mime }), { contentType: mime, upsert: false }));
      return insert("document", { name: data.name, mime, file: storagePath, sha256: digest, size: binary.byteLength, date: data.date || "", study_type: data.study_type || "Informe", visit: data.visit, identity_verified: false, identity_note: "", status: "stored" }, data.patient);
    }
    if (path.startsWith("file/")) {
      const doc = record(await row(path.split("/")[1]));
      await demo(doc.patient);
      return unwrap(await client.storage.from("clinical-documents").createSignedUrl(doc.file, 300)).signedUrl;
    }
    if (path === "documents/identity") {
      const old = await row(data.id);
      await demo(old.patient_id);
      return update(data.id, { identity_verified: !!data.confirmed, identity_note: data.note, reviewed_by: session.user.email });
    }
    if (path === "calculations") {
      await demo(data.patient);
      const weight = record(await row(data.weight)), height = record(await row(data.height));
      if (weight.status !== "approved" || height.status !== "approved" || weight.unit !== "kg" || height.unit !== "cm") throw new Error("Selecciona peso en kg y talla en cm aprobados.");
      const w = Number(weight.value), h = Number(height.value);
      let value, unit, source = data.formula === "bmi" ? "who-bmi" : "mifflin-1990";
      if (data.formula === "bmi") {
        value = w / (h / 100) ** 2;
        unit = "kg/m\xB2";
      } else {
        const age = Number(data.age);
        if (!data.applicability || age < 19 || age > 78 || !["female", "male"].includes(data.sex)) throw new Error("Completa y confirma las entradas de Mifflin.");
        value = 10 * w + 6.25 * h - 5 * age + (data.sex === "male" ? 5 : -161);
        unit = "kcal/d\xEDa en reposo";
      }
      return insert("calculation", { visit: data.visit, formula: data.formula, source, inputs: { weight_kg: w, height_cm: h, age: data.age || null, sex: data.sex || null, age_source: data.age_source || "", selection_reason: data.date_reason || "" }, value: Math.round(value * 100) / 100, unit, dependencies: [{ id: weight.id, rev: weight.rev }, { id: height.id, rev: height.rev }], status: "approved", author: session.user.email }, data.patient);
    }
    if (path.startsWith("audit/")) {
      const id = path.split("/")[1];
      return unwrap(await client.from("audit_events").select("actor_id,created_at,reason,before_data,after_data,action").eq("workspace_id", workspace).eq("record_id", id).order("created_at", { ascending: false })).map((x) => ({ actor: x.actor_id || "Sistema", at: x.created_at, reason: x.reason || x.action, before_json: JSON.stringify(x.before_data), after_json: JSON.stringify(x.after_data) }));
    }
